...
class flight_system:
	...
	def main1(self,states):
		self.intents = ['flight','airfare','airline','ground_service','ground_fare']
	# intent details will contain propertires like intent state size and intent action size

		self.intent_details = dict()
		self.intent_details['flight'] = [5, 13]
		self.intent_details['airfare'] = [5, 13]
		self.intent_details['airline'] = [3, 8]
		self.intent_details['ground_service'] = [2, 6]
		self.intent_details['ground_fare'] = [2, 6]
		...
		for i in self.intents:
			...
			elif self.intent_details[i][0] == 5:
